...
public class OrderTaxTest {
    ...
    public void testTaxOnlineOrder() {
        Order order1 = new Order("5", Location.ONLINE);     // create a new Order called order1 and pass an ID and a LOCATION ENUM
        order1.processCart(cart1);      // prints the report and calculates subtotal
        assertEquals(0.0, order1.getTax(), .001);       // assert equals what should the outcome be

        Order order2 = new Order("4", Location.ONLINE);
        order2.processCart(cart1);
        assertEquals(0.0, order2.getTax(), .001);
    }

    @Test
    public void testTaxEuropeOrder() {
        Order order1 = new Order("1", Location.EUROPE);
    ...
    public void testTaxUSAOrder() {
        Order order1 = new Order("1", Location.USA);
        order1.processCart(cart1);
        assertEquals(0.0,order1.getTax(), .001);

        Order order2 = new Order("2", Location.USA);
        order2.processCart(cart2);
