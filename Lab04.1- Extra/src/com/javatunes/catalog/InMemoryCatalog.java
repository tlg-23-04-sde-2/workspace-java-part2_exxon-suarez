...
public class InMemoryCatalog implements Catalog {
    ...
    public Collection<MusicItem> findByCategory(MusicCategory category) {
        Collection<MusicItem> result = new ArrayList<>();                   // create new Collection instance of MusicItem named result that is an ArrayList
        for(MusicItem item: catalogData) {                                  // for-each item in catalogData
            if(item.getMusicCategory().equals(category)) {                  // if the current item has a catagory that equals the catagory we were passed
                result.add(item);                                           // add that item to our new results ArrayList
            }
        }
        return result;
    }
    ...
    public Collection<MusicItem> selfTitleAlbums() {                // dont need any incoming parameters // we have all the data in the catalogData Collections
    ...
    public Collection<MusicItem> getItemsAtSpecifiedPrice(double maxPrice) {            // pass user input maxPrice
        Collection<MusicItem> result = new ArrayList<>();
        return result;
    }
     ...
     */
    public int genreCount(MusicCategory genre) {                                // pass as parameter the clients genre input
        int count = 0;
        return count;
    }
     ...
     */
    public double averagePrice() {                                              // we dont need any parameters from user // all iin the catalog
        return 0.0;
    }
